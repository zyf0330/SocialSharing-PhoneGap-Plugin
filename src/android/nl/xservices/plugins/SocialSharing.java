...
public class SocialSharing extends CordovaPlugin {
    ...
    private Uri getFileUriAndSetType(Intent sendIntent, String dir, String fileUrl, String subject, int nthFile) throws IOException {
        // we're assuming an image, but this can be any filetype you like
        String localFileUrl = fileUrl.concat("");

        if (fileUrl.startsWith("http") || fileUrl.startsWith("www/")) {
            String filename = getFileName(fileUrl);
            sendIntent.setType(getMIMEType(filename));
            localFileUrl = "file://" + dir + "/" + filename;
            ...
            final String fileName = fileUrl.substring(fileUrl.indexOf("df:") + 3, fileUrl.indexOf(";data:"));
            final String fileType = fileUrl.substring(fileUrl.indexOf(";data:") + 6, fileUrl.indexOf(";base64,"));
            final String encodedImg = fileUrl.substring(fileUrl.indexOf(";base64,") + 8);
            ...
            sendIntent.setDataAndType(Uri.fromFile(new File(fileUrl)), type);
        }
    ...
    private String getMIMEType(String fileName) {
        ...
        String extension = fName.substring(dotIndex + 1).toLowerCase();
        if (extension.trim() == "") return type;
